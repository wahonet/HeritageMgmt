...
let editCtx = null; // 编辑台账上下文（原隐式全局，现模块内声明）
...
export async function selectProject(pid) {
  ...
  const data = await get(`/project/${pid}`);
  renderProject(data);
  // 同步侧栏高亮
...
// ===== 视图 / 标签切换 =====
export function switchView(mode) { state.viewMode = mode; localStorage.setItem('viewMode', mode); selectProject(state.currentProjectId); }
export function switchTab(code) { state.activeTab = code; document.querySelectorAll('.tab-btn').forEach(b => b.classList.toggle('active', b.dataset.tab === code)); renderTabContent(); }
...
// 阶段文档面板（改用 state.lastData，取消冗余 refetch）
export function openStage(code) {
...
// 上传
export function openUpload(pid, docType, typeName) {
...
export function closeUpload() { $('#uploadModal').classList.add('hidden'); state.uploadCtx = null; }
...
export async function saveEdit() {
  ...
  const j = await put('/project/' + editCtx.pid, data);
...
export async function ocrScan(pid, btn) {
  ...
  try {
    const j = await post('/ocr/scan?project_id=' + pid, null, true);
    if (j.ok) {
      const f = j.fields || {};
...
// ---------- 文件结构树 ----------
// 阶段名从 /api/config 的 workflow 派生（取代前端硬编码 STAGE_NAMES）
function stageNames() {
  const m = {};
  for (const s of (state.config?.workflow || [])) m[s.code] = s.name;
  return m;
}
export async function showFileTree(pid) {
  $('#treeTitle').textContent = '文件结构';
  $('#treeBody').innerHTML = '<div class="loading">加载中…</div>';
  $('#treeModal').classList.remove('hidden');
  const data = await get(`/project/${pid}/tree`);
  ...
  for (const d of data) {
    const stageName = sn[d.stage] || d.stage || '';
...
export async function openDocByName(pid, name) {
  // 先查文档ID
  const d = await get(`/project/${pid}`);
...
// ---------- 删除分类 ----------
export async function deleteDocType(pid, docType, label) {
  if (!confirm(`确认删除「${label}」分类下的全部文件？\n\n将同时删除数据库记录和磁盘文件，不可恢复。`)) return;
  try {
    const j = await del(`/project/${pid}/doctype/${docType}`);
    if (j.ok) {
      const e = document.getElementById(`ftf_${docType}`);
      if (e) e.remove();
...
// ---------- 导出报告 ----------
export function exportReport(pid) {
