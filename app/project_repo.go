...
func GetProject(id int64) (*domain.Project, error) {
	row := db.QueryRow("SELECT "+projectCols+" FROM projects WHERE id=?", id)
	return scanProject(row)
}

func ListProjects(unitID int64, status, q string) ([]domain.Project, error) {
...
func UpdateProjectFields(id int64, sets string, vals []interface{}) error {
	vals = append(vals, id)
	_, err := db.Exec("UPDATE projects SET "+sets+" WHERE id=?", vals...)
	return err
}

// CreateProject 新建工程，返回新ID
func CreateProject(unitID int64, name, ptype, status string) (int64, error) {
...
func SetProjectFolder(id int64, folder string) error {
	_, err := db.Exec("UPDATE projects SET folder=? WHERE id=?", folder, id)
	return err
}

// RestoreProjectRecord 取消软删除标记（从回收站恢复）
func RestoreProjectRecord(id int64) error {
	_, err := db.Exec("UPDATE projects SET deleted=0 WHERE id=?", id)
	return err
}

// PurgeProjectRecord 彻底删除工程及其文档记录
func PurgeProjectRecord(id int64) error {
	if _, err := db.Exec("DELETE FROM documents WHERE project_id=?", id); err != nil {
		return err
	}
...
func ProjectIDsByUnit(uid int64) []int64 {
	var pids []int64
...
func ListRecycled() ([]domain.RecycledProject, error) {
	rows, err := db.Query(`SELECT p.id,p.name,p.folder,p.ptype,p.status,u.name FROM projects p LEFT JOIN units u ON u.id=p.unit_id WHERE COALESCE(p.deleted,0)=1 ORDER BY p.id DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
